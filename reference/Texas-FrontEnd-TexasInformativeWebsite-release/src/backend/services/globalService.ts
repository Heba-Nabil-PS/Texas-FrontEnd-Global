...
export async function getSingleCategoryMedia(
  pageCode: string,
  categoryCode: string,
  locale: string
) {
  const url = `${process.env.API_URL}/API/${process.env.PROJECT_CODE}/AdvancedContent/${process.env.COUNTRY_CODE}/${pageCode}/${locale}/Category/${categoryCode}/Media`;
  const data = await fetcher(url);
  return data;
}
